...
def checkpoint(model, filename):
    ...
    #on va également exporter nos données dans le json
...
if __name__ == "__main__":
   ...
   #Appelle la fonction qui sélectionne automatiquement le périphérique
    ...
    fasttext_path =  os.path.join(os.path.dirname(os.path.realpath(__file__)),"models","fst250_10K_all.model")
    ...
    train_loader,nb_intent_train,nb_slots_train = get_loader_IntentAndSlot(train_dataset_path,vocab,intent2id,slot2id,batch_size)
    val_loader,nb_intent_test,nb_slots_test = get_loader_IntentAndSlot(test_dataset_path,vocab,intent2id,slot2id,batch_size)
    print(f"train intent:{nb_intent_train} slots:{nb_slots_train}")
    print(f"test intent:{nb_intent_test} slots:{nb_slots_test}")

    nb_slots = max((nb_slots_train,nb_slots_test))
    nb_intent = max((nb_intent_train,nb_intent_test))

    # Initialisation du modèle Transformer pour la détection d’intention et d’étiquetage de slots
    model = Model(fasttext_path,vocab,nb_intent=nb_intent,nb_slots=nb_slots,nhead=2,nhid=250,nlayers=1,dropout=0.4)


    model_name = "Tranformer_IntentSlots_model.pt"
    ...
    for epoch in range(epochs):
        ...
        if len(history["intent_train"]) >= 10:
            plt.figure(figsize=(10,6))
            plt.plot(history["intent_train"], label="Intent Train Loss")
            plt.plot(history["intent_val"], label="Intent Val Loss")
            plt.plot(history["slot_train"], label="Slot Train Loss")
            plt.plot(history["slot_val"], label="Slot Val Loss")
            plt.xlabel("Epochs")
            plt.ylabel("Loss")
            plt.title("Évolution des pertes d'entraînement et validation")
            plt.legend()
            plt.grid(True)

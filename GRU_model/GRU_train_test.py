...
if __name__ == "__main__":
    ...
    model_name = "GRU_IntentSlots_model.pt"
    ...
    for epoch in range(epochs):
        ...
        for utt_x,intent_y,slot_y in tqdm(train_loader):
            ...
            intent_out,slot_out = model(utt_x) # [N,nb_intent], [N,Seq,nbSlots]
            intent_loss = loss_f_intent(intent_out,intent_y)
            intent_train_loss += intent_loss.item() * intent_out.size(0)
            
            count += intent_out.size(0)

            slot_out_p = torch.permute(slot_out,(0,2,1)) # necessaire pour crossentropy loss [N,nbslots,seq] 

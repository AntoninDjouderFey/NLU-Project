...
def plotter(history:dict[str,float]):
    ...
    ax2.legend()
    plt.grid(True)
    plt.savefig("GRU_zero_shot_training_data.png", dpi=150)
    plt.close()
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if len(history["intent_train"]) >= 5:
            plotter(history)
    print("--------------------------------------")
    print("Model best validation loss :",best_val_loss/count_val)

"""
 intent_train_loss: 0.0033 intent_val_loss: 1.3443
 slot_train_loss: 0.0008 slot_val_loss: 0.0677
 time: 40.3221s
intent precision: 0.8195
 intent recall: 0.8102 intent fbeta: 0.8076

slot precision: 0.9387
 slot recall: 0.9401 slot fbeta: 0.9381
"""